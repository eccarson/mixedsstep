...
% specified problem, using tighter bounds.
%
% Input:
...
[resultsm, basis_infom] = mixed_ca_lanczos_bounds_tight(A, v, s,basis_info, options);
...
semilogy(1:m-1, results.orthactual(1:m-1),'b-', 1:m-1, results.orthbound(1:m-1),'b--', ...
    1:m-1, results.orthboundtight(1:m-1),'b:', ...
    1:m-1, resultsm.orthactual(1:m-1),'r-', 1:m-1, resultsm.orthbound(1:m-1),'r--', ...,
    1:m-1, resultsm.orthboundtight(1:m-1), 'r:', 1:m-1,results.Gamma_orth(1:m-1),'k-');
...
semilogy(1:m-1, results.normactual(1:m-1),'b-', 1:m-1, results.normbound(1:m-1),'b--', ...
    1:m-1, results.normboundtight(1:m-1), 'b:',...
    1:m-1, resultsm.normactual(1:m-1),'r-', 1:m-1, resultsm.normbound(1:m-1),'r--', ...
    1:m-1, resultsm.normboundtight(1:m-1), 'r:', 1:m-1,results.Gamma_norm(1:m-1),'k-');
tt = strcat('Normality, $s=$ ', num2str(s), ", ", basis_info.type, ' basis');
title(tt,'Interpreter','latex','FontSize',18);
% Uncomment below to plot legend
 legend('measured, uniform', 'bound, uniform', 'tight bound, uniform','measured, mixed', 'bound, mixed', 'tight bound, mixed','$\bar{\Gamma}_k$', 'Interpreter', 'latex','FontSize',14); xlabel('Iteration','Interpreter','latex','FontSize',14)
...
1:m-1, results.colboundtight(1:m-1), 'b:',...
    1:m-1, resultsm.colactual(1:m-1),'r-', 1:m-1, resultsm.colbound(1:m-1),'r--', ...
    1:m-1, resultsm.colboundtight(1:m-1), 'r:', 1:m-1,results.Gamma_colbound(1:m-1),'k-');
...
semilogy(1:m-1, results.diffactual(1:m-1),'b-', 1:m-1, results.diffbound(1:m-1),'b--',...
    1:m-1, results.diffboundtight(1:m-1), 'b:',...
    1:m-1, resultsm.diffactual(1:m-1),'r-', 1:m-1, resultsm.diffbound(1:m-1),'r--', ...
    1:m-1, resultsm.diffboundtight(1:m-1), 'r:', 1:m-1,results.Gamma_diffbound(1:m-1),'k-');
