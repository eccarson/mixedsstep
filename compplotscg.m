...
%
%   See cg_example.m for example usage.
%
% Last edited by: Erin Carson, 2021
%
function compplotscg(A, b, s, basis_info, options)

% Run the classical CG algorithm with the specifiec parameters
results = cg(A, b, zeros(size(A,1),1), options);

% Run the uniform precision s-step CG algorithm with the specified
% parameters
resultsca = cacg(A, b, s, zeros(size(A,1),1), basis_info, options);

% Run the mixed precision s-step CG algorithm with the specified parameters
resultscam = cacg_mixed(A, b, s, zeros(size(A,1),1), basis_info, options);
