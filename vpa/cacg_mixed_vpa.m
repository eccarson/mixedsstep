...
% the Gram matrix.
% Uses MATLAB vpa for quadruple precision.
%
% Input:
...
function results = cacg_mixed_vpa(A, b, s, x0, basis_info, options)
...
while its < options.xlim
    ...
    G = vpa(double([P,R]'),32)*vpa(double([P,R]),32);
    
    if (k>0)
        gammax = max(cond(double(G)), gammax);
    end
    ...
    for j = 1:s
        ...
        alpha(its) = ( ( r_c(:,j)'* double(vpa(G,32)*vpa(double((r_c(:,j))))) ) / ( p_c(:,j)'*double(vpa(G,32)*vpa(double((Tt*p_c(:,j))),32)) ) );
        ...
        % Gram matrix in quad
        beta(its) = (( r_c(:,j+1)'* double(vpa(G,32)*vpa(double((r_c(:,j+1))))) )/ ( r_c(:,j)'* double(vpa(G,32)*vpa(double((r_c(:,j))))) ));
        ...
        err = double(vpa(double(x(:,its+1)),32) - vpa(options.truesol,32));
        num = sqrt(double(vpa(double(err'),32)*vpa(double(A),32)*vpa(double(err),32)));
        denom = sqrt(double(vpa(options.truesol',32)*vpa(double(A),32)*vpa(options.truesol,34)));
