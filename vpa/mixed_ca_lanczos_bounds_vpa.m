...
%
function [results, basis_info] = mixed_ca_lanczos_bounds_vpa(A, v, s, basis_info, options)
...
while m < options.xlim
    ...
    G = vpa(double([V,U]),32)'*vpa(double([V,U]),32);
    ...
    for j = 1:s
        ...
        alpha(m) = (vcoeff(:,j)'*double(vpa(G,32)*vpa(double(ucoeff(:,j)),32))); % apply G to vector in quadruple precision
        wcoeff = ucoeff(:,j) - alpha(m)*vcoeff(:,j);
        w = ([V,U]*wcoeff);
        beta(m+1) = sqrt(wcoeff'* double(vpa(G,32)*vpa(double(wcoeff),32))); % apply G to vector in quadruple precision
        ...
        if(m>1)
            results.orthactual(m) = vpa(beta(m+1),64)*abs(vpa(v(:,m),64)'*vpa(v(:,m+1),64));
            results.orthbound(m) =  e0*sigma;
            results.normactual(m) = abs(vpa(vpa(v(:,m+1),64)'*vpa(v(:,m+1),64)-1,64));
            results.normbound(m) = e0/2;
            results.colactual(m) = norm(vpa( vpa(A,64)*vpa(v(:,m),64)-(vpa(beta(m+1),64)*vpa(v(:,m+1),64) + vpa(beta(m),64)*vpa(v(:,m-1),64) + vpa(alpha(m),64)*vpa(v(:,m),64)),64));
            results.colbound(m) = e1*sigma;
            results.diffactual(m) = abs(vpa(beta(m+1),64)^2 + vpa(alpha(m),64)^2 + vpa(beta(m),64)^2 - norm(vpa(A,64)*vpa(v(:,m),64))^2);
